...
class GenericEvolver(BaseEvolver):
    ...
    >>> # create a recorder
    >>> Record = namedtuple('record', ['score', 'age', 'nchildren'])
    >>> start_record = lambda indiv, score: Record(score, 0, 0)
    >>> update_record = (
    ...     lambda indiv, rec, nchild: Record(rec[0], rec[1]+1, rec[2]+nchild)
    ... )
    ...
    >>> join = op.add
    >>> operators = Operators(
    ...     recorder, estimator, selector, crossover, mutator, join, policy
    ... )
    ...
    def evolve_generation(
        ...
        recorder, estimator, selector, crossover, mutator, join, policy = operators
        ...
        if len(mates) != n_mating_groups:
            raise RuntimeError
        recombined = crossover(individuals, records, mates, **kwargs)
        ...
        records_updated = recorder.update(
            individuals, records, mates, crossover.broodsize
        )
        # join individuals and records and select a new population
        joined_individuals = join(individuals, children)
        joined_records = join(records_updated, child_records)
